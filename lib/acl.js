...
ACL.prototype.removeUserRoles = function(userId, roles, callback){
  ...
  this.connector.getNodesWithLabelsAndProperties(['User'], {uid: userId}, function(err, userNodes) {
    ...
    this.connector.getNodeRelationships(userNode._id, {types: ['BELONGS_TO']}, function(err, relationships) {
      // @todo: If relationships between userId and each role, erase them
...
ACL.prototype.userRoles = function(userId, callback){
  // @todo: Get all relationships of type 'BELONGS_TO' coming from that user
