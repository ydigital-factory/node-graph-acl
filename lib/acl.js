  ...
  async.map(
    ...
    function(role, cb){
      _this._createRole(role, function (err, createdRole) {
        createdRoles.shift(createdRole);
        cb(null, true);
      });
    },
    function(err, results){
      async.map(
        ...
        function(resource, cb){
          _this._createResource(resource, function (err, createdResource) {
            createdResources.shift(createdResource);
            cb(null, true);
          });
        },
        function(err, results){
          createdRoles.forEach(function (role) {
            createdResources.forEach(function (resource) {
              //     create relationship with properties: 'create': true, etc
              _this._createRelationship(role._id, resource._id, 'HAS_ACCESS_TO', permissionsFormatted, cb);
            });
          });
        }
      );
