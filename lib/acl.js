...
/**
  ...
  @todo: alternative complex syntax:
...
/**
  ...
  Remove permissions to resources from a given role.

  @param {String} role - role to remove permissions from.
  @param {String|Array} resources - resource(s) to remove permisisons from.
  @param {String|Array} permissions - permission(s) to remove from the roles over the resources.
  @param {Function} callback - Callback called when finished.
*/
          ...
          async.map(
            ...
            function(role, cb){
              _this.connector.getNodeRelationships(role._id, {types: ['HAS_ACCESS_TO']}, function(err, relationships) {
                var cleanedRelationships = [];

                relationships.forEach(function (relationship) {
                  _this.connector._getParameters(relationship, function (err, cleaned) {
                    cleanedRelationships.push(cleaned);
                  });
                });

                cb(null, cleanedRelationships);
              });
            },
            function(err, results){
              console.log(results);
...
/**
  ...
  Examples:
    ...
    connector._createRelationship(1, 2, 'TYPE', {p1:'v1', p2:'v2'}, callback);
